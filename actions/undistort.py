...
def undistort_image(shot, undistorted_shots, original, interpolation,
    ...
    if projection_type in ['perspective', 'brown', 'fisheye', 'fisheye_opencv']:
        ...
        if original.shape[2] == 4:
            labels = original[:, :, 3]
            #print (labels.shape)
            undistorted_l = cv2.remap(labels, map1, map2, interpolation)      
            undistorted_image = scale_image(undistorted_l, max_size)
        ...
        for subshot in undistorted_shots:
            undistorted   = render_perspective_view_of_a_panorama(image, shot, subshot, mint)
