...
def add_views_to_depth_pruner(data, neighbors, dp):
    for shot in neighbors:
        ...
        height, width = depth.shape
        image = scale_down_image(color_image[:, :, :3], width, height)
        try:
        	labels = scale_down_image(color_image[:, :, 3], width, height, cv2.INTER_NEAREST)
        except IndexError:
        	labels = scale_down_image(color_image[:, :, 2], width, height, cv2.INTER_NEAREST)
